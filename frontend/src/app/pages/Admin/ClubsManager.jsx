...
import {
    Search, Plus, Filter, Users, CheckCircle, XCircle, Edit3,
    Trash2, Shield, Clock, MoreHorizontal, TrendingUp, Globe, Lock,
    Eye, RefreshCw, Settings, BarChart3, X, Crown, Tag
} from 'lucide-react'
import { useClubs } from '../../hooks/useClubs'

const STATUS_CONFIG = {
    active: { label: 'Active', cls: 'bg-emerald-50 text-emerald-700 border-emerald-200', icon: <CheckCircle size={12} /> },
...
export default function ClubsManager() {
    const {
        clubs, approveClub, suspendClub, deleteClub, updateClub, createClub
    } = useClubs()
    ...
    const [form, setForm] = useState({ name: '', description: '', type: 'Interest Group', category: 'Technology', isPrivate: false, coverPhoto: '' })
    ...
    const openCreate = () => {
        setEditingClub(null)
        setForm({ name: '', description: '', type: 'Interest Group', category: 'Technology', isPrivate: false, coverPhoto: 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&q=80&w=1200&h=400' })
        setIsDrawerOpen(true)
    }

    const openEdit = (club) => {
        setEditingClub(club)
        setForm({ name: club.name, description: club.description, type: club.type, category: club.category, isPrivate: club.isPrivate, coverPhoto: club.coverPhoto })
        setIsDrawerOpen(true)
    ...
    return (
        <div className="max-w-7xl mx-auto py-6 px-1 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight mb-1">Clubs Management</h1>
                    <p className="text-slate-500 font-medium">Approve, manage, and monitor all clubs and groups.</p>
                </div>
                <button
                    onClick={openCreate}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
                {[
                    { label: 'Total Groups', value: stats.total, icon: <Shield size={20} className="text-indigo-600" />, bg: 'bg-indigo-50' },
                    { label: 'Active', value: stats.active, icon: <CheckCircle size={20} className="text-emerald-600" />, bg: 'bg-emerald-50' },
                    { label: 'Pending Approval', value: stats.pending, icon: <Clock size={20} className="text-amber-600" />, bg: 'bg-amber-50' },
                    { label: 'Total Members', value: stats.members.toLocaleString(), icon: <Users size={20} className="text-blue-600" />, bg: 'bg-blue-50' },
                ].map(s => (
            ...
            {stats.pending > 0 && (
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-6 flex items-center gap-3">
                    ...
                    <button onClick={() => setStatusFilter('pending')} className="text-xs font-bold text-amber-700 underline hover:no-underline">
                        Review now
                    </button>
                </div>
            )}
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="divide-y divide-slate-100">
                    ...
                    {filtered.map(club => {
                        ...
                        return (
                            <div key={club.id} className="grid grid-cols-12 gap-3 px-5 py-4 hover:bg-slate-50 transition-colors items-center relative">
                                {/* Group Info */}
                                <div className="col-span-4 flex items-center gap-3 min-w-0">
                                    <div className="w-12 h-12 rounded-xl overflow-hidden shrink-0 bg-slate-100">
                                        <img src={club.coverPhoto} alt={club.name} className="w-full h-full object-cover" />
                                    </div>
                                ...
                                <div className="col-span-2">
                                    <span className={`text-[11px] font-bold px-2 py-1 rounded-full ${TYPE_COLORS[club.type] || 'bg-slate-100 text-slate-600'}`}>
                                        {club.type}
                                    </span>
                                </div>
                                ...
                                <div className="col-span-1 text-center">
                                    <div className="flex items-center justify-center gap-1">
                                        ...
                                        <span className="text-sm font-bold text-slate-700">{club.membersCount?.toLocaleString()}</span>
                                    </div>
                                </div>

                                {/* Status */}
                                <div className="col-span-2 flex items-center justify-center">
                                    <span className={`flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-full border ${statusCfg.cls}`}>
                                ...
                                <div className="col-span-1 flex items-center justify-end gap-1 relative">
                                    ...
                                    {settingsMenuId === club.id && (
                                        <div className="absolute right-0 top-8 z-20 bg-white border border-slate-200 rounded-xl shadow-xl w-44 py-1">
                                            ...
                                            <button
                                                onClick={() => { setSettingsMenuId(null) }}
                                                className="flex items-center gap-2.5 w-full px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors"
                                            >
                                            ...
                                            {club.status === 'active' && (
                                                <button
                                                    onClick={() => { suspendClub(club.id); setSettingsMenuId(null) }}
                                            ...
                                            {club.status === 'suspended' && (
                                                <button
                                                    onClick={() => { approveClub(club.id); setSettingsMenuId(null) }}
                ...
                <div className="fixed inset-0 z-50 flex justify-end">
                    ...
                    <div className="relative bg-white shadow-2xl w-full max-w-[500px] flex flex-col h-full border-l border-slate-200 animate-in slide-in-from-right duration-300">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                            <div>
                                <h2 className="text-lg font-extrabold text-slate-900">{editingClub ? 'Edit Club' : 'Create New Club'}</h2>
                                <p className="text-xs text-slate-500 font-medium mt-0.5">{editingClub ? 'Update club details.' : 'Fill in details to create a new community.'}</p>
                            </div>
                            <button onClick={() => setIsDrawerOpen(false)} className="p-2 bg-slate-50 hover:bg-slate-200 rounded-full text-slate-500 transition-colors">
                                <X size={18} />
                            </button>
                        </div>
                        <div className="flex-1 overflow-y-auto p-6 space-y-5 bg-slate-50/40">
                            <form id="club-form" onSubmit={handleSubmit} className="space-y-5">
                                {[
                                    { label: 'Club Name *', field: 'name', type: 'text', placeholder: 'e.g. NYC Alumni Network' },
                                    { label: 'Cover Photo URL', field: 'coverPhoto', type: 'url', placeholder: 'https://...' },
                                ].map(f => (
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        ...
                                        <select value={form.type} onChange={e => setForm(p => ({ ...p, type: e.target.value }))} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none font-medium">
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <select value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value }))} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none font-medium">
                                            {['Technology', 'Business', 'Geography', 'Class Year', 'Department', 'Interest', 'Alumni'].map(c => <option key={c}>{c}</option>)}
                                        </select>
                                    </div>
                                </div>
                                <label className="flex items-center gap-3 cursor-pointer bg-white border border-slate-200 rounded-xl px-4 py-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500 font-medium">Members must request to join</p>
                                    </div>
                                </label>
                                {form.coverPhoto && (
                                    <div className="h-36 rounded-xl overflow-hidden bg-slate-100 border border-slate-200">
                                        <img src={form.coverPhoto} alt="" className="w-full h-full object-cover" onError={e => e.target.style.opacity = 0} />
                                    </div>
                                )}
                            </form>
                        </div>
                        <div className="p-5 border-t border-slate-100 bg-white flex justify-end gap-3 shrink-0">
                            <button onClick={() => setIsDrawerOpen(false)} className="px-5 py-2.5 font-bold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-all">
                                Cancel
                            </button>
                            <button form="club-form" type="submit" className="px-6 py-2.5 font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-md transition-all hover:-translate-y-0.5">
