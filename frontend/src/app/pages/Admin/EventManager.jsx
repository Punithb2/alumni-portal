    ...
    return (
        <div className="max-w-7xl mx-auto py-6 animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight mb-1">Event Management</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filteredEvents.map(event => (
                    <div key={event.id} className="relative group/manager">
                        ...
                        <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover/manager:opacity-100 transition-opacity">
                            <button onClick={() => handleOpenSheet(event)} className="bg-white/95 p-2 text-slate-700 hover:text-indigo-600 rounded-xl shadow-md border border-slate-200 transition-colors">
                                Edit
                            </button>
                            <button onClick={() => handleDelete(event.id)} className="bg-white/95 p-2 text-slate-700 hover:text-rose-600 rounded-xl shadow-md border border-slate-200 transition-colors">
                                Delete
                            </button>
                        </div>
                    </div>
                ...
                {filteredEvents.length === 0 && (
                    <div className="col-span-full py-20 text-center text-slate-500 bg-white border border-slate-200 border-dashed rounded-3xl shadow-sm">
                        ...
                        <p className="text-slate-500">Try adjusting your search criteria or create a new event.</p>
                    </div>
                )}
            </div>
                ...
                <div className="fixed inset-0 z-50 flex justify-end">
                    ...
                    <div className="relative w-full max-w-[500px] bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 border-l border-slate-200">
                        <div className="flex items-center justify-between p-6 border-b border-slate-100 shrink-0 bg-white">
                            <h2 className="text-xl font-extrabold text-slate-900">{editingEvent ? 'Edit Event' : 'Create New Event'}</h2>
                            <button
                                onClick={() => setSheetOpen(false)}
                        ...
                        <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50">
                            <form id="event-form" onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-5">
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Venue / Link</label>
                                        <input
                                            required
                                            type="text"
                                ...
                                <div className="grid grid-cols-2 gap-5">
                                    ...
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Ticket Price (₹0 = Free)</label>
                                        <input
                                            required
                                            type="number"
                                            min="0"
