...
const AlumniJobBoard = () => {
  ...
  const allJobs = useMemo(() => {
    ...
    const jobs = [...postedJobs, ...dummys, ...defaultMock]
  ...
  const filteredJobs = useMemo(() => {
    let list = activeTab === 'saved' ? allJobs.filter(j => savedJobIds.has(j.id)) : 
                activeTab === 'myposts' ? postedJobs : allJobs
  ...
  }, [allJobs, activeTab, search, typeFilter, locationFilter, expFilter, industryFilter, savedJobIds, postedJobs, alumniRecommendedOnly])

  // Handlers
  const handlePost = (job) => {
    setPostedJobs([{ ...job, id: 'mypost-'+Date.now(), postedBy: user?.name || 'Alumni', postedByRole: 'Alumni', alumniPosted: true }, ...postedJobs])
    setShowModal(false)
    ...
    <div className="max-w-[1500px] mx-auto min-h-[calc(100vh-100px)] flex flex-col pb-8 font-sans px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl">
             ...
             <button 
                ...
                <span className={`block w-4 h-4 rounded-full bg-white absolute top-0.5 transition-all ${openToWork ? 'left-5' : 'left-0.5'}`}></span>
             </button>
          </div>
          <button
      ...
      {openToWork && (
          <div className="mb-6 bg-indigo-50 border border-indigo-200 p-4 rounded-xl flex items-center justify-between">
              <div>
                  <h3 className="font-bold text-indigo-900 text-sm">Your profile is visible to recruiters!</h3>
                  <p className="text-xs text-indigo-700 mt-1">Target Roles: Frontend Developer, Software Engineer � Location: Any</p>
              </div>
              <button className="text-xs font-bold text-indigo-700 bg-white px-3 py-1.5 rounded-lg border border-indigo-200 hover:bg-indigo-50">Edit Preferences</button>
          </div>
      )}
                 ...
                 <div className="space-y-4">
                     <div className="p-4 border border-slate-100 bg-slate-50 rounded-xl flex items-center justify-between">
                         <div className="flex items-center gap-3">
                             <div className="w-10 h-10 bg-indigo-100 text-indigo-700 rounded-full flex items-center justify-center font-bold">SJ</div>
                             <div>
                                 <h4 className="font-bold text-slate-800 text-sm">Sarah Jenkins (Student)</h4>
                                 <p className="text-xs text-slate-500">Requested referral for: Software Engineer at Innovate</p>
                             </div>
                         </div>
                         <div className="flex gap-2">
                             <button className="px-3 py-1.5 bg-white border border-slate-300 text-slate-700 text-xs font-bold rounded-lg hover:bg-slate-50">View Message</button>
                             <button className="px-3 py-1.5 bg-indigo-600 text-white text-xs font-bold rounded-lg hover:bg-indigo-700">Approve</button>
                         </div>
                     </div>
                     <div className="p-4 border border-slate-100 bg-slate-50 rounded-xl flex items-center justify-between">
                         <div className="flex items-center gap-3">
                             <div className="w-10 h-10 bg-teal-100 text-teal-700 rounded-full flex items-center justify-center font-bold">MK</div>
                             <div>
