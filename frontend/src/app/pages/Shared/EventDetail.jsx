              ...
              {event.capacity && (
                <div className="mt-4 pt-4 border-t border-slate-100">
                  <div className="flex justify-between text-xs font-bold mb-2">
                    <span
                      className={
                        event.capacity - (event.attendees || 0) < 10
                          ? 'text-rose-500'
                          : 'text-slate-500'
                      }
                    >
                    ...
                    <span className="text-slate-400">
                      {Math.round(((event.attendees || 0) / event.capacity) * 100)}% Full
                    </span>
                  </div>
                  <div className="w-full bg-slate-100 rounded-full h-1.5">
                    <div
                      className={`h-1.5 rounded-full ${event.capacity - (event.attendees || 0) < 10 ? 'bg-rose-500' : 'bg-indigo-500'}`}
                      style={{
                        width: `${Math.min(100, ((event.attendees || 0) / event.capacity) * 100)}%`,
                      }}
                    ></div>
                  </div>
                </div>
