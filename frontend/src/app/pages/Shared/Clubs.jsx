...
const SECTIONS = [
    { type: 'Interest Group', label: 'Featured Interest Groups', desc: 'Connect around shared passions and industries' },
                    ...
                    {viewingGroupId ? (
                        ...
                        <div className="space-y-12">
                            {SECTIONS.map(section => {
                                const sectionClubs = filteredClubs.filter(g => g.type === section.type && g.status === 'active')
                                ...
                                return (
                                    <section key={section.type}>
                                        <div className="flex items-center justify-between mb-5">
                                            ...
                                            <button className="text-sm font-bold text-indigo-600 hover:text-indigo-700 transition-colors">See all</button>
                                        </div>
                        ...
                        <div className="space-y-6">
                            {myGroups.length === 0 ? (
                                <div className="bg-white rounded-2xl border border-slate-200 border-dashed p-16 text-center">
                                    ...
                                    <h3 className="text-lg font-bold text-slate-700 mb-1">You haven't joined any groups yet</h3>
                                    <p className="text-slate-500 text-sm mb-5">Explore the discover tab to find communities that match your interests.</p>
                                    <button
                                    ...
                                    {pendingCount > 0 && (
                                        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center gap-3">
                                            ...
                                            <p className="text-sm text-amber-800 font-bold">
                                                You have <span className="underline">{pendingCount} pending join request{pendingCount > 1 ? 's' : ''}</span>. Awaiting group admin approval.
                                            </p>
                                        </div>
                                    )}
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
