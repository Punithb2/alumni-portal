...
import {
    Users, ShieldCheck, Lock, Globe, MessageSquare, Image as ImageIcon,
    MoreHorizontal, ThumbsUp, Pin, Send, Search, Crown, Shield,
    Star, UserMinus, UserCheck, CheckCircle, X, Calendar, MapPin, Briefcase,
    Building2, Clock, ChevronDown, ChevronUp, Smile, Paperclip, Hash,
    BookOpen, Info, ArrowLeft, Mail, Share2
} from 'lucide-react'
...
// 1) FEED TAB
// ═══════════════════════════════════════════════════════════════
const FeedTab = ({ clubId, user, posts, isAdmin, onCreatePost, onLike, onComment, onDelete, onPin, likedPosts }) => {
                    ...
                    <div className="flex-1 space-y-3">
                        ...
                        {feedImage && (
                            <div className="relative inline-block mt-2">
                                <img src={feedImage} alt="Upload preview" className="w-32 h-32 object-cover rounded-xl border border-slate-200" />
                                <button onClick={() => setFeedImage(null)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 shadow">
                                    <X size={12} />
                                </button>
                            </div>
                        )}
                ...
                <div key={post.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    {post.isPinned && (
                        <div className="bg-indigo-50 px-6 py-2.5 flex items-center gap-2 border-b border-indigo-100">
                            ...
                            <span className="text-xs font-bold text-indigo-600 uppercase tracking-wider">Pinned by Admin</span>
                        </div>
                    )}
                    <div className="p-6">
                        ...
                        <p className="text-slate-700 leading-relaxed mb-4 whitespace-pre-wrap text-sm">{post.content}</p>

                        {post.image && (
                            <div className="mb-4 rounded-xl overflow-hidden border border-slate-100">
                                <img src={post.image} alt="Post attachment" className="w-full max-h-64 object-cover" />
                            </div>
                        )}
                        ...
                        <div className="flex items-center gap-1 border-t border-slate-100 pt-4">
                            ...
                            >
                                ...
                                <span className="hidden sm:inline">{likedPosts.has(post.id) ? 'Liked' : 'Like'}</span>
                            </button>
                            ...
                            >
                                ...
                                <span className="hidden sm:inline">{sharedPostIds.has(post.id) ? 'Copied' : 'Share'}</span>
                            </button>
                        </div>
                                ...
                                {post.comments?.map(c => (
                                    <div key={c.id} className="group">
                                        <div className="flex gap-3 items-start">
                                            <img src={c.avatar} alt={c.author} className="w-8 h-8 rounded-full object-cover shrink-0 mt-0.5" />
                                            <div className="flex-1">
                                                ...
                                                <div className="flex items-center gap-3 mt-1 ml-2 text-[12px] font-semibold text-slate-500">
                                                    <button onClick={() => setReplyBoxes(prev => ({ ...prev, [c.id]: !prev[c.id] }))} className="hover:text-slate-800 transition-colors">Reply</button>
                                            ...
                                            <div className="ml-11 mt-3 space-y-3">
                                                {c.replies?.map(r => (
                                                    <div key={r.id} className="flex gap-2 items-start">
                                                        <img src={r.avatar} alt={r.author} className="w-6 h-6 rounded-full object-cover shrink-0" />
                                                        <div className="flex-1">
                                                            <div className="bg-slate-50 border border-slate-100 rounded-xl px-3 py-2 inline-block">
                                                                <p className="text-[12px] font-bold text-slate-900 mb-0.5">{r.author}</p>
                                                                <p className="text-[13px] text-slate-700">{r.text}</p>
                                                            </div>
                                                            <div className="mt-0.5 ml-2 text-[11px] font-semibold text-slate-500">
                                                                <span className="text-slate-400 font-normal">{r.time || 'Just now'}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ))}
                                                    ...
                                                    <div className="flex gap-2 mt-2">
                                                        ...
                                                        <div className="flex-1 flex gap-2">
                                                            <input
                                                                ...
                                                                onKeyDown={e => e.key === 'Enter' && handleReplySubmit(post.id, c.id)}
...
// 2) CHAT TAB
// ═══════════════════════════════════════════════════════════════
const ChatTab = ({ clubId, messages, onSend }) => {
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-slate-50/60">
                ...
                {(messages || []).map(msg => (
                    <div key={msg.id} className={`flex gap-3 ${msg.isMe ? 'flex-row-reverse' : ''}`}>
                        {!msg.isMe && (
                            msg.avatar
                                ? <img src={msg.avatar} alt={msg.sender} className="w-8 h-8 rounded-full object-cover shrink-0 mt-1" />
                        ...
                        <div className={`flex flex-col gap-1 max-w-[75%] ${msg.isMe ? 'items-end' : ''}`}>
                            ...
                            <div className={`px-4 py-3 rounded-2xl text-sm leading-relaxed shadow-sm ${msg.isMe
                            ...
                            <div className={`flex gap-1.5 text-[11px] font-medium px-1 ${msg.isMe ? '' : ''}`}>
                                ...
                                {msg.isMe && <span className="text-indigo-400 font-bold">{msg.status || 'Sent'}</span>}
                            </div>
                        </div>
                    </div>
                ))}
...
// 3) MEMBERS TAB
// ═══════════════════════════════════════════════════════════════
const MembersTab = ({ clubId, members, joinRequests, isAdmin, onApproveRequest, onRejectRequest, onRemoveMember, onPromoteMember }) => {
    const [search, setSearch] = useState('')
    const [showRequests, setShowRequests] = useState(true)

    const filtered = (members || []).filter(m =>
        m.name.toLowerCase().includes(search.toLowerCase()) || m.title?.toLowerCase().includes(search.toLowerCase())
    )
    const grouped = {
                ...
                <div className="bg-white rounded-2xl border border-amber-200 shadow-sm overflow-hidden">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <span className="bg-amber-500 text-white text-xs font-black px-2 py-0.5 rounded-full">{pendingRequests.length}</span>
                        </div>
                            ...
                            {pendingRequests.map(req => (
                                <div key={req.id} className="p-5 flex items-start gap-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {req.message && (
                                            <p className="text-xs text-slate-600 italic bg-slate-50 rounded-lg px-3 py-2 mt-2">"{req.message}"</p>
                                        )}
                    ...
                    return (
                        <div key={role}>
                            ...
                            <div className="divide-y divide-slate-100">
                                {group.map(m => (
                                    <div key={m.id} className="px-5 py-4 flex items-center justify-between gap-3 hover:bg-slate-50 transition-colors">
                                        <div className="flex items-center gap-3 min-w-0">
                                            <div className="relative shrink-0">
                                                <img src={m.avatar} alt={m.name} className="w-11 h-11 rounded-full object-cover" />
                                                <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white ${m.isOnline ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                                            </div>
                                            <div className="min-w-0">
                                                <div className="flex items-center gap-2">
                                                    <p className="text-sm font-bold text-slate-900 truncate">{m.name}</p>
                                                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${ROLE_BADGE[m.role]}`}>
                                                        {m.role}
                                                    </span>
                                                </div>
                                                <p className="text-xs text-slate-500 font-medium truncate">{m.title}</p>
                                                <p className="text-[11px] text-slate-400 font-medium">Joined {m.joinedAt}</p>
                                            </div>
                                        </div>
        ...
        <div className="space-y-5">
            ...
            {(!events || events.length === 0) && (
                <div className="bg-white rounded-2xl border border-slate-200 p-16 text-center">
                    ...
                    <p className="text-sm text-slate-500">Check back later for events organized by this group.</p>
                </div>
            )}
            <div className="grid grid-cols-1 gap-5">
                {(events || []).map(evt => (
                    <div key={evt.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col sm:flex-row hover:shadow-md transition-all">
                        ...
                        <div className="p-5 flex-1 flex flex-col">
                            <div className="flex items-start justify-between mb-2">
                                <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${evt.type === 'Virtual' ? 'bg-blue-50 text-blue-700' : 'bg-emerald-50 text-emerald-700'}`}>
                                    {evt.type}
                                </span>
                            </div>
                            ...
                            <div className="mt-auto flex items-center gap-3">
                                <button
                                    ...
                                    className={`px-4 py-2 text-sm font-bold rounded-xl transition flex items-center gap-2 ${rsvped.has(evt.id)
                                        ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                                        : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm'
                                        }`}
                                >
                                    {rsvped.has(evt.id) ? <><CheckCircle size={15} /> RSVPed</> : <><Calendar size={15} /> RSVP</>}
                                </button>
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="space-y-4">
                {(jobs || []).map(job => (
                    <div key={job.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 hover:shadow-md transition-all hover:border-indigo-200 group">
                        <div className="flex items-start gap-4">
                            <div className="w-14 h-14 rounded-xl overflow-hidden border border-slate-100 shrink-0 bg-slate-50">
                                <img src={job.logo} alt={job.company} className="w-full h-full object-cover" onError={e => { e.target.style.display = 'none' }} />
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex items-start justify-between gap-3">
                                    ...
                                    <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full shrink-0 ${JOB_TYPE_COLORS[job.type] || 'bg-slate-100 text-slate-600'}`}>
                                        {job.type}
                                    </span>
                                </div>
                                ...
                                <div className="mt-4 flex items-center gap-3">
                                    ...
                                    <span className="text-xs text-slate-400 font-medium ml-auto">Posted by {job.postedBy}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                ...
                <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {[
                        ...
                        { label: 'Privacy', value: group.isPrivate ? 'Private' : 'Public', icon: group.isPrivate ? '🔒' : '🌐' },
                        { label: 'Members', value: group.membersCount?.toLocaleString(), icon: '👥' },
                        { label: 'Created', value: group.createdAt, icon: '📅' },
                        { label: 'Status', value: group.status?.charAt(0).toUpperCase() + group.status?.slice(1), icon: '✅' },
                    ].map(info => (
                        <div key={info.label} className="bg-slate-50 rounded-xl p-4">
                            <p className="text-lg mb-1">{info.icon}</p>
                            <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-0.5">{info.label}</p>
            ...
            {group.tags?.length > 0 && (
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {group.tags.map(t => (
                            <span key={t} className="bg-indigo-50 text-indigo-700 text-sm font-bold px-4 py-2 rounded-full">
                                #{t}
                            </span>
                        ))}
                    </div>
                ...
                <div className="divide-y divide-slate-100">
                    {(group.admins || []).map(admin => (
                        <div key={admin.id} className="p-5 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                <img src={admin.avatar} alt={admin.name} className="w-12 h-12 rounded-full object-cover ring-2 ring-indigo-50" />
                                <div>
...
const ClubDetail = ({ groupId }) => {
    ...
    const {
        clubs, posts, members, joinRequests, messages,
        joinedIds, pendingIds, clubEvents, clubJobs,
        joinClub, leaveClub, cancelRequest,
        createPost, deletePost, likePost, addComment, pinPost,
        approveJoinRequest, rejectJoinRequest, removeMember, promoteMember,
        sendMessage,
    } = useClubs()
    ...
    const isGroupAdmin = isAdmin || (members[group?.id] || []).some(
    ...
    const groupJobs = clubJobs[group?.id] || []

    const handleJoinToggle = () => {
        if (isJoined) { leaveClub(group.id); return }
        if (isPending) { cancelRequest(group.id); return }
        joinClub(group.id)
        if (!group.isPrivate) {
    ...
    const joinBtnConfig = isJoined
        ? { label: 'Leave Group', cls: 'bg-slate-100 text-slate-700 hover:bg-red-50 hover:text-red-600 border border-slate-200' }
        : isPending
            ? { label: 'Cancel Request', cls: 'bg-amber-50 text-amber-700 hover:bg-amber-100 border border-amber-200' }
            : group.isPrivate
                ? { label: 'Request to Join', cls: 'bg-indigo-600 text-white hover:bg-indigo-700 ring-2 ring-indigo-600 ring-offset-2 ring-offset-slate-900 border-transparent' }
                : { label: 'Join Group', cls: 'bg-indigo-600 text-white hover:bg-indigo-700 ring-2 ring-indigo-600 ring-offset-2 ring-offset-slate-900 border-transparent' }
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div className="h-48 md:h-60 w-full bg-slate-100 relative">
                    ...
                    <div className="absolute top-4 right-4">
                        {group.isPrivate
                            ? <div className="bg-slate-900/80 backdrop-blur-sm text-white text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-2 border border-white/10"><Lock size={13} /> Private</div>
                            : <div className="bg-white/90 backdrop-blur-sm text-slate-700 text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-2 shadow-sm"><Globe size={13} className="text-emerald-600" /> Public</div>
                        }
                    </div>
                </div>
                ...
                <div className="px-6 md:px-8 pb-0 pt-4 relative">
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-5 -mt-16 md:-mt-20 relative z-10">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                                <span className="bg-white/90 backdrop-blur-md text-slate-900 text-[10px] font-extrabold uppercase tracking-wider px-2.5 py-1 rounded-md shadow-md">{group.type}</span>
                                <span className="bg-indigo-600/90 backdrop-blur-md text-white text-[10px] font-extrabold uppercase tracking-wider px-2.5 py-1 rounded-md shadow-md">{group.category}</span>
                            </div>
                            <h1 className="text-2xl md:text-3xl font-black text-white leading-tight drop-shadow-md mb-2">{group.name}</h1>
                            <p className="text-slate-200 text-sm font-medium line-clamp-2 drop-shadow-sm">{group.description}</p>
                        </div>
                        ...
                        <div className="flex flex-col items-start md:items-end gap-3 shrink-0 mt-2 md:mt-0">
                            <div className="flex items-center gap-3 bg-white/10 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/20">
                                <div className="flex -space-x-2">
                                    {(group.memberAvatars || []).slice(0, 5).map((av, i) => (
                                        <img key={i} src={av} alt="" className="w-8 h-8 rounded-full border-2 border-slate-900 object-cover" />
                                    ))}
                                </div>
                                <span className="text-white text-sm font-bold">+{group.membersCount.toLocaleString()}</span>
                            </div>
                            <button
                            ...
                            >
                                {isJoined ? <><ShieldCheck size={16} className="text-emerald-500" /> Joined</> : isPending ? <><Clock size={16} /> Requested</> : <><Users size={16} /> {joinBtnConfig.label}</>}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="mt-5 flex items-center gap-1 overflow-x-auto scrollbar-hide border-b border-slate-200 -mx-6 md:-mx-8 px-6 md:px-8">
                        {TABS.map(tab => (
                            ...
                            >
                                {React.cloneElement(tab.icon, { className: activeTab === tab.id ? 'text-indigo-600' : 'text-slate-400' })}
                                {tab.label}
                                {tab.id === 'members' && groupRequests.length > 0 && isGroupAdmin && (
                                    <span className="ml-1 w-4 h-4 flex items-center justify-center bg-amber-500 text-white text-[9px] font-black rounded-full">{groupRequests.length}</span>
                                )}
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                <div className="flex-1 min-w-0">
                    ...
                    {activeTab === 'events' && (
                        <EventsTab events={groupEvents} isAdmin={isGroupAdmin} />
                    )}
                    {activeTab === 'about' && (
                    ...
                    <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
                        ...
                        <div className="space-y-4 text-sm font-medium text-slate-600">
                             <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                                 <span className="flex items-center gap-2"><Globe size={16} className="text-slate-400" /> Type</span>
                                 <span className="text-slate-900 font-bold">{group.isPrivate ? 'Private' : 'Public'}</span>
                             </div>
                             <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                                 <span className="flex items-center gap-2"><Users size={16} className="text-slate-400" /> Members</span>
                                 <span className="text-slate-900 font-bold">{group.membersCount?.toLocaleString()}</span>
                             </div>
                             <div className="flex justify-between items-center">
                                 <span className="flex items-center gap-2"><Crown size={16} className="text-amber-500" /> Created By</span>
