...
const ClubDiscoverCard = ({ group, onView, isJoined, isPending, onJoin, onCancelRequest }) => {
    ...
    const handleBtnClick = (e) => {
        e.stopPropagation()
        if (isPending) { onCancelRequest?.(group.id); return }
        if (isJoined) { onView(group.id); return }
        onJoin?.(group.id)
    }

    const btnConfig = isJoined
        ? { label: 'View Group', icon: <CheckCircle size={14} />, cls: 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100' }
        : isPending
            ? { label: 'Requested', icon: <Hourglass size={14} />, cls: 'bg-amber-50 text-amber-700 border-amber-200 hover:bg-red-50 hover:text-red-600 hover:border-red-200' }
            : group.isPrivate
                ? { label: 'Request to Join', icon: <Lock size={14} />, cls: 'bg-indigo-600 text-white border-indigo-600 hover:bg-indigo-700' }
                : { label: 'Join Group', icon: <Users size={14} />, cls: 'bg-indigo-600 text-white border-indigo-600 hover:bg-indigo-700' }
        ...
        >
            ...
            <div className="relative h-36 bg-slate-100 w-full overflow-hidden shrink-0">
                ...
                <div className="absolute bottom-3 left-4 flex gap-2">
                    ...
                    <span className={`${catColor}/90 backdrop-blur-md text-white text-[10px] font-extrabold uppercase tracking-wider px-2 py-1 rounded-md shadow-sm`}>
                        {group.category}
                    </span>
                </div>
            </div>
                ...
                {group.tags?.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-4">
                        {group.tags.slice(0, 3).map(tag => (
                            <span key={tag} className="bg-slate-100 text-slate-600 text-[10px] font-bold px-2 py-0.5 rounded-full">
                                #{tag}
                            </span>
                        ))}
                    </div>
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                        <div className="flex -space-x-2 shrink-0">
                            {(group.memberAvatars || []).slice(0, 4).map((av, i) => (
                                <img key={i} src={av} alt="" className="w-6 h-6 rounded-full border-2 border-white object-cover shadow-sm" />
                            ))}
                        </div>
