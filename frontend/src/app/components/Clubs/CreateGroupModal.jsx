...
const CATEGORIES = ['Technology', 'Business', 'Arts', 'Sports', 'Geography', 'Class Year', 'Department', 'Interest', 'Alumni']
const TYPES = [
    ...
    { value: 'Cohort', label: 'Cohort / Class Year', desc: 'Classmates from a specific batch or department' },
]
const PRIVACY_OPTIONS = [
    { value: 'public', icon: <Globe size={16} className="text-emerald-600" />, label: 'Public', desc: 'Anyone in the network can view and join.' },
    { value: 'private', icon: <Lock size={16} className="text-indigo-600" />, label: 'Private', desc: 'Visible, but members must request to join.' },
    { value: 'invite_only', icon: <EyeOff size={16} className="text-slate-600" />, label: 'Hidden', desc: 'Invisible. Invitation required to join.' },
]
...
const CreateGroupModal = ({ isOpen, onClose }) => {
    ...
    const [form, setForm] = useState({
        ...
        category: 'Technology',
        privacy: 'public',
        coverPhoto: 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&q=80&w=1200&h=400',
        tags: '',
        invited: '',
    })
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <div className="flex gap-1">
                                {[1, 2].map(s => (
                                    <div key={s} className={`h-1 w-8 rounded-full transition-all ${step >= s ? 'bg-white' : 'bg-white/30'}`} />
                                ))}
                            </div>
                            <span className="text-white/70 text-xs font-bold">Step {step} of 2</span>
                        </div>
                        <h2 className="text-lg font-extrabold">{step === 1 ? 'Group Details' : 'Privacy & Visuals'}</h2>
                        <p className="text-indigo-100 text-xs mt-0.5">
                            {step === 1 ? 'Tell us about your group community.' : 'Set access controls and a cover image.'}
                        </p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-5 bg-slate-50/40">
                        ...
                        <>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="space-y-2">
                                    {TYPES.map(t => (
                                        <label key={t.value} className={`flex items-start gap-3 p-3.5 rounded-xl border cursor-pointer transition-all ${form.type === t.value ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200 bg-white hover:border-indigo-200'}`}>
                                            <div className="relative mt-0.5">
                                                <input type="radio" name="type" value={t.value} checked={form.type === t.value} onChange={handle('type')} className="sr-only" />
                                                <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ${form.type === t.value ? 'border-indigo-600' : 'border-slate-300'}`}>
                                                    {form.type === t.value && <div className="w-2 h-2 rounded-full bg-indigo-600" />}
                                                </div>
                                            </div>
                                            <div>
                                                <p className={`text-sm font-bold ${form.type === t.value ? 'text-indigo-700' : 'text-slate-800'}`}>{t.label}</p>
                            ...
                            <div className="space-y-1.5">
                                <label className="block text-sm font-bold text-slate-800 flex items-center gap-1.5"><Tag size={13} /> Tags <span className="text-slate-400 font-normal">(comma separated)</span></label>
                                <input
                                    type="text"
                                    value={form.tags}
                    ...
                    {step === 2 && (
                        <>
                            {/* Cover Photo */}
                            <div className="space-y-2">
                                <label className="block text-sm font-bold text-slate-800 flex items-center gap-1.5"><ImgIcon size={13} /> Cover Photo</label>
                                <input
                                    type="file"
                                    accept="image/*"
                                    onChange={(e) => {
                                ...
                                />
                                {form.coverPhoto && form.coverPhoto !== 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&q=80&w=1200&h=400' && (
                                    <div className="rounded-xl overflow-hidden h-32 bg-slate-100 border border-slate-200">
                                        <img src={form.coverPhoto} alt="Preview" className="w-full h-full object-cover" onError={e => e.target.style.opacity = 0} />
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="space-y-2">
                                    {PRIVACY_OPTIONS.map(opt => (
                                        <label key={opt.value} className={`flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-all ${form.privacy === opt.value ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200 bg-white hover:border-indigo-200'}`}>
                                            <div className="relative mt-0.5">
                                                <input type="radio" name="privacy" value={opt.value} checked={form.privacy === opt.value} onChange={handle('privacy')} className="sr-only" />
                                                <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ${form.privacy === opt.value ? 'border-indigo-600' : 'border-slate-300'}`}>
                                                    {form.privacy === opt.value && <div className="w-2 h-2 rounded-full bg-indigo-600" />}
                                                </div>
                                            </div>
                                            <div className="flex items-start gap-2.5">
                                                {opt.icon}
                                                <div>
                                                    <p className={`text-sm font-bold ${form.privacy === opt.value ? 'text-indigo-700' : 'text-slate-800'}`}>{opt.label}</p>
                            ...
                            <div className="bg-white rounded-xl border border-slate-200 p-4">
                                <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Group Preview</p>
                                <div className="flex items-center gap-3">
                                    <div className="w-14 h-14 rounded-xl overflow-hidden bg-slate-100 shrink-0">
                                        <img src={form.coverPhoto} alt="" className="w-full h-full object-cover" onError={() => { }} />
                                    </div>
                                    <div>
                                        <p className="font-bold text-slate-900 text-sm">{form.name || 'Group Name'}</p>
                                        <p className="text-xs text-slate-500 font-medium">{form.type} · {form.category}</p>
                                        <p className="text-[11px] text-slate-400 mt-0.5">{form.privacy === 'public' ? '🌐 Public' : form.privacy === 'private' ? '🔒 Private' : '👁️ Hidden'}</p>
                                    </div>
                                </div>
                            </div>
                        </>
