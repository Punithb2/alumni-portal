        ...
        <div className="w-full bg-white rounded-2xl shadow-sm border border-slate-200 p-3 mb-6">
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap">
                ...
                <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide">
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2.5 rounded-xl transition-all font-bold text-sm whitespace-nowrap ${activeTab === 'discover'
                            ? 'bg-indigo-50 text-indigo-700'
                            : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
                            }`}
                    >
                        <Compass size={16} className={activeTab === 'discover' ? 'text-indigo-600' : 'text-slate-400'} />
                        Discover Groups
                    </button>
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2.5 rounded-xl transition-all font-bold text-sm whitespace-nowrap relative ${activeTab === 'my_groups'
                            ? 'bg-emerald-50 text-emerald-700'
                            : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
                            }`}
                    >
                        <Users size={16} className={activeTab === 'my_groups' ? 'text-emerald-600' : 'text-slate-400'} />
                        My Groups
                        {pendingCount > 0 && (
            ...
            {activeTab === 'discover' && (
                <div className="mt-3 pt-3 border-t border-slate-100 flex items-center gap-3 flex-wrap">
                    ...
                    {/* Filter Pills */}
                    <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide">
                        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider hidden md:block pr-1">Type:</span>
                        <button
                            onClick={() => setSelectedFilter(null)}
                            className={`px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all border ${selectedFilter === null
                        ...
                        {FILTERS.map(f => (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all border ${selectedFilter === f.id
