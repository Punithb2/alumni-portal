...
const EventCard = ({ event, rsvpdEvents, onEventClick, isAdmin }) => {
    ...
    return (
        <div className="group bg-white rounded-2xl shadow-sm hover:shadow-md border border-slate-200 overflow-hidden transition-all flex flex-col h-full">
            {/* Banner */}
            <button onClick={onEventClick} className="relative h-36 sm:h-40 bg-slate-100 overflow-hidden shrink-0 block outline-none origin-center p-0 m-0 border-0 w-full text-left">
                <img
                ...
                <div className="space-y-1.5 mb-4 text-xs text-slate-600 flex-1">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <span>{event.attendees || 0} {event.attendees === 1 ? 'attendee' : 'attendees'}</span>
                        </div>
                        {event.capacity && (
                ...
                <div className="mt-auto pt-3 border-t border-slate-100 flex items-center justify-between">
                    {isAdmin ? (
                        <div className="w-full py-1.5 flex gap-2">
                            ...
                            <button onClick={onEventClick} className="text-slate-400 font-medium text-xs flex-1 text-center bg-slate-50 hover:bg-slate-100 transition-colors rounded-lg py-1.5">View Details</button>
                        </div>
                    ) : isRsvpd ? (
                        <button
                        ...
                        <button
                            onClick={onEventClick}
                            className={`w-full py-2 text-sm font-semibold rounded-xl transition-all flex items-center justify-center gap-2 shadow-sm ${isSoldOut
