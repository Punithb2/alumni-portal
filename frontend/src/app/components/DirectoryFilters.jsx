...
import { ChevronDown, Check, Filter, LayoutGrid, List, Map as MapIcon, X, Search } from 'lucide-react'
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="space-y-2.5 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
        {options.map((option) => (
          <label key={option.value} className="flex items-center gap-3 group cursor-pointer lg:hover:bg-gray-50 lg:p-1 lg:-ml-1 lg:rounded-md transition-colors">
            <div
              className={clsx(
...
export function SortDropdown({ label, value, options, onChange }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="font-medium whitespace-nowrap">{label}: <span className="text-gray-900">{options.find(o => o.value === value)?.label || 'Newest'}</span></span>
...
export default function DirectoryFilters({
  ...
  const renderActiveFilterPills = () => {
    ...
    if (willingToMentor) {
      pills.push(
        <span key="mentor" className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
          Willing to Mentor
          <button onClick={() => onWillingToMentorChange(false)} className="hover:bg-blue-200 p-0.5 rounded-full transition-colors">
    ...
    filterOptions.forEach(section => {
      ...
      activeValues.forEach(val => {
        ...
        if (option) {
          pills.push(
            <span key={`${section.id}-${val}`} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-medium bg-gray-100 text-gray-800 border border-gray-200">
          ...
          <div className="px-5 sm:px-8 py-6">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <h1 className="text-2xl sm:text-3xl border-gray-300 font-bold text-gray-900 border-none outline-none">Directory</h1>
        ...
        <div className="bg-white border md:rounded-2xl border-gray-200 mb-6 shadow-sm flex flex-col relative z-20">
          <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between px-4 sm:px-6 py-4 gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3 sm:gap-4">
              ...
              >
                <Filter className={clsx("w-4 h-4", isFilterPanelOpen || activeFilterCount > 0 ? "text-white" : "text-gray-500")} />
                ...
                {onWillingToMentorChange && (
                  <div className="flex flex-col">
                    ...
                    <label className="flex items-center justify-between cursor-pointer group p-3.5 bg-white border border-gray-200 rounded-xl hover:border-blue-300 hover:bg-blue-50/50 transition-all shadow-sm">
                      <span className="text-sm font-bold text-gray-900 group-hover:text-blue-800 transition-colors">Willing to Mentor</span>
