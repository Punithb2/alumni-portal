...
const CATEGORY_LABELS = { scholarship: 'Scholarship', infrastructure: 'Infrastructure', emergency: 'Emergency', research: 'Research', health: 'Health', sports: 'Sports', event: 'Events' };
...
export const CampaignCard = ({ campaign, isAdmin = false, onEdit, onDelete, onCampaignClick }) => {
    const { id, title, story, raised, goal, donorCount, imageUrl, deadline, featured, urgent, category, status, campaignType } = campaign;
    ...
    return (
        <div className={`bg-white rounded-2xl border overflow-hidden shadow-sm hover:shadow-md transition-all group flex flex-col h-full hover:-translate-y-1 duration-300 ${urgent ? 'border-rose-300' : isCompleted ? 'border-slate-200' : 'border-slate-200'}`}>
            <div className="relative h-48 overflow-hidden bg-slate-100">
                ...
                {isAdmin && (
                    <div className="absolute bottom-3 right-3 flex gap-1.5">
                        <button onClick={(e) => { e.stopPropagation(); onEdit(campaign); }} className="bg-white/90 p-1.5 text-slate-700 hover:text-violet-600 rounded-lg shadow-sm backdrop-blur-sm transition-colors hover:bg-white">
                            <Edit size={14} />
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); onDelete(campaign.id); }} className="bg-white/90 p-1.5 text-slate-700 hover:text-rose-600 rounded-lg shadow-sm backdrop-blur-sm transition-colors hover:bg-white">
                            <Trash2 size={14} />
                        </button>
                    </div>
                )}
            ...
            <div className="p-5 flex flex-col flex-1 bg-white">
                {category && (
                    <span className={`self-start text-[10px] font-bold px-2 py-0.5 rounded border mb-2 ${CATEGORY_COLORS[category] || 'bg-slate-50 text-slate-600 border-slate-200'}`}>
                        {CATEGORY_LABELS[category] || category}
                    </span>
                )}
                <h3 className="text-base font-bold text-slate-900 mb-1.5 line-clamp-1 group-hover:text-violet-700 transition-colors">{title}</h3>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-100 mt-auto">
                    ...
                    {!isAdmin ? (
                        ...
                        <button onClick={() => onCampaignClick(campaign)} className="text-slate-500 font-medium text-xs hover:text-slate-800 transition-colors underline underline-offset-2">
                            Preview
                        </button>
                    )}
                </div>
