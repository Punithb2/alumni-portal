    ...
    return (
        <div className={`w-full ${className}`}>
            <div className="flex justify-between items-end text-sm mb-1.5 font-medium">
                ...
                {goal > 0 && <span className="text-slate-500 text-xs leading-none">
                    {isParticipation ? `${current.toLocaleString()} / ${goal.toLocaleString()}` : `₹${current.toLocaleString()} / ₹${goal.toLocaleString()}`}
                </span>}
            </div>
