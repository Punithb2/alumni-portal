    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 mb-4">
        <div className="flex justify-between items-center mb-4 px-1">
            <div className="flex items-center gap-2">
                ...
                {subtitle && (
                    <span className="text-[12px] text-slate-400 font-medium ml-1 hidden sm:inline">- {subtitle}</span>
                )}
            </div>
        ...
        <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-none px-1">
            {users.map((u, i) => (
                <button key={i} title={u.title} className="flex-shrink-0 flex flex-col items-center gap-1.5 group focus:outline-none">
                    <div className="relative">
                        <div className={`p-0.5 rounded-full ${u.online ? 'bg-gradient-to-tr from-indigo-500 via-purple-500 to-pink-500' : 'bg-slate-200'}`}>
                    ...
                    <span className="text-[11px] font-semibold text-slate-600 truncate max-w-[56px]">{u.title}</span>
                </button>
            ))}
        </div>
    </div>
...
const ImageGrid = ({ images }) => {
    ...
    if (images.length === 1) return (
        <div className="w-full max-h-[480px] overflow-hidden bg-slate-100 cursor-pointer">
            <img src={images[0]} alt="post" className="w-full h-full object-cover hover:scale-[1.02] transition-transform duration-500" />
        </div>
    )
        ...
        <div className="grid grid-cols-2 gap-0.5 bg-slate-100 cursor-pointer max-h-[360px]">
            {images.map((src, i) => (
                <div key={i} className="overflow-hidden bg-slate-200">
                    <img src={src} alt={`post ${i}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500 min-h-[240px]" />
                </div>
            ))}
        </div>
    )
    // 3+ image mosaic
    return (
        <div className="grid gap-0.5 bg-slate-100 cursor-pointer max-h-[400px]" style={{ gridTemplateColumns: '2fr 1fr' }}>
            <div className="overflow-hidden bg-slate-200">
                <img src={images[0]} alt="post 0" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500 min-h-[400px]" />
            </div>
            <div className="flex flex-col gap-0.5">
                {images.slice(1, 3).map((src, i) => (
                    <div key={i} className="overflow-hidden bg-slate-200 flex-1 relative">
                        <img src={src} alt={`post ${i + 1}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
                        {i === 1 && images.length > 3 && (
...
const MoreOptions = ({ onClose }) => {
    ...
    return (
        <div ref={ref} className="absolute right-0 top-8 z-50 w-52 bg-white border border-slate-200 shadow-lg rounded-2xl py-1.5 overflow-hidden animate-fade-in">
            ...
            <div className="flex gap-2.5 items-start">
                ...
                <div className="flex-1 min-w-0">
                    <div className="inline-block bg-slate-50 border border-slate-100 rounded-2xl rounded-tl-sm px-3.5 py-2 text-[13px] text-slate-800 max-w-full">
                        <p className="font-bold text-slate-900 text-[12px] hover:text-indigo-600 cursor-pointer mb-0.5 truncate">{c.author}</p>
                    ...
                    <div className="flex items-center gap-3 mt-1 ml-1 text-[11px] font-semibold text-slate-400">
                        ...
                        {depth === 0 && (
                            <button onClick={() => setReplyOpen(!replyOpen)} className="hover:text-slate-700 transition-colors">
                                Reply
                            </button>
                        )}
...
export const FeedPost = ({ post, currentUser }) => {
    ...
    const [comments, setComments] = useState(Array.isArray(post.mockComments) ? post.mockComments : [])
    ...
    useEffect(() => {
        const handler = (e) => {
            if (reactionRef.current && !reactionRef.current.contains(e.target)) setShowReactionPicker(false)
        }
        document.addEventListener('mousedown', handler)
        return () => document.removeEventListener('mousedown', handler)
    }, [])
    ...
    const topReactionEmojis = Object.entries(reactions).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([e]) => e)
    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 mb-4 overflow-hidden hover:shadow-md transition-shadow duration-300">
                    ...
                    <div className="min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            {post.verified && post.verifiedType === 'official' && (
                                <span title="Verified Official" className="text-[13px]">✅</span>
                            )}
                            {post.verified && post.verifiedType === 'alumni' && (
                                <span title="Verified Alumni" className="inline-flex items-center justify-center w-4 h-4 bg-indigo-600 rounded-full text-white text-[9px] font-bold leading-none">✓</span>
                            )}
                        </div>
                        <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                            <p className="text-[12px] text-slate-500 font-medium leading-tight truncate max-w-[160px] sm:max-w-none">{post.role}</p>
                            <span className="w-1 h-1 bg-slate-300 rounded-full flex-shrink-0" />
                            <span className="text-[11px] text-slate-400 font-medium flex-shrink-0">{post.time}</span>
                        </div>
                        {/* Post type badge */}
                        <span className={`inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full text-[11px] font-semibold ${postType.color}`}>
            ...
            <div className="px-4 pb-3">
                <p className={`text-[14px] text-slate-700 leading-relaxed whitespace-pre-wrap ${!isContentExpanded && requiresTruncation ? 'line-clamp-3' : ''}`}>
                    {post.content}
                </p>
                {requiresTruncation && (
                    <button
                    ...
                    <div className="flex items-center gap-1.5 cursor-pointer group">
                        <div className="flex">
                            {topReactionEmojis.map((e, i) => (
                                <span key={i} className="text-[16px] -ml-1 first:ml-0">{e}</span>
                            ))}
                        </div>
                        <span className="text-[13px] text-slate-500 font-medium group-hover:text-indigo-600 transition-colors">
                            {myReaction ? `You${totalReactionCount > 1 ? ` and ${formatCount(totalReactionCount - 1)} others` : ''}` : formatCount(totalReactionCount)}
                        </span>
                    </div>
                    <button
            ...
            <div className="px-1 py-0.5 border-t border-slate-100 flex items-center">
                ...
                <div ref={reactionRef} className="relative flex-1">
                    <button
                        ...
                        onMouseEnter={() => { reactionTimerRef.current = setTimeout(() => setShowReactionPicker(true), 400) }}
                        onMouseLeave={() => { clearTimeout(reactionTimerRef.current) }}
            ...
            <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl overflow-hidden animate-modal-in">
                ...
                <div className="px-5">
                    <textarea
                        ref={textareaRef}
                        value={text}
                        onChange={(e) => { if (e.target.value.length <= MAX_CHARS) { setText(e.target.value); autoResize(e) } }}
                        placeholder={
                            postType === 'QUESTION' ? "Ask your network something..." :
                            postType === 'ACHIEVEMENT' ? "Share a win or milestone..." :
                            postType === 'EVENT' ? "Share an event or opportunity..." :
                            "What's on your mind?"
                        }
                        rows={3}
                ...
                {images.length > 0 && (
                    <div className="px-5 py-2 flex gap-2 overflow-x-auto scrollbar-none">
                        {images.map((src, i) => (
                            <div key={i} className="relative flex-shrink-0 w-20 h-20 rounded-xl overflow-hidden border border-slate-200 group">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {text.length > 0 && (
                            <div className="relative w-7 h-7 flex items-center justify-center">
                                ...
                                {remaining < 50 && (
                                    <span className={`absolute text-[9px] font-bold ${remaining < 20 ? 'text-red-500' : 'text-amber-500'}`}>
                                        {remaining}
                                    </span>
                                )}
                            </div>
